...
@include('components.break')
        ...
        <div class="grid lg:grid-cols-3 gap-5">
            ...
            <div class="card p-5 mt-3">
                    <h3> <small class="font-bold">Total:( {{(count($teachers))}} )</small></h3>
                    <table class="table-sorter table table_striped w-full mt-0">
                        ...
                        <?php $user = Profil::where('token', $teacher->token)->first(); ?>   
                            <tr>
                                <td>{{ $teacher->id }}</td>
                                <td class="font-bold">{{ $teacher->fname }}</td>
                                <td class="font-bold">{{ $teacher->lname }}</td>
                                <td class="font-bold">{{ $user->phone }}</td>
                                <td class="font-bold">{{ $user->email }}</td>
                                <td class="font-bold">
                                @if($teacher->sexe == 1)Homme @else Femme @endif
                                </td>
                                <td class="text-center">
                                <div class="inline-flex ltr:ml-auto rtl:mr-auto">
                                    <a href="{{URL::route('profileTeacher', $teacher->token)}}" class="btn btn-icon btn_success">
                                        <span class="la la-user"></span>
                                    </a>
                                    <a href="{{ URL::current() . '?id=' . $teacher->id }}" class="btn btn-icon btn_primary ltr:ml-2 rtl:mr-2">
